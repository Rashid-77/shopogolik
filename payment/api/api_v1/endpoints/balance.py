...
REQUEST_TIME_BACKET = Histogram(
    "balance_request_latency_seconds", "Time spent processing request", ["endpoint"]
)
...
def create_deposit(
    ...
    logger.info(
        f" user_id={current_user.id}, admin={current_user.is_superuser}, "
    ...
    if current_user.id == bal_in.user_id or current_user.is_superuser:
        ...
        acc = crud.money_account.create(
            db, bal_in.user_id, bal_in.depos_uuid, bal_in.amount
        )
        if acc is None:
            raise HTTPException(status_code=422, detail="Unprocessable Entity.")
        return acc
...
def do_withdraw(
    ...
    if current_user.id == bal_in.user_id or current_user.is_superuser:
        if bal_in.amount < 0:
            raise HTTPException(status_code=422, detail="Unprocessable Entity.")
        acc = crud.money_account.withdraw(
            db, bal_in.user_id, bal_in.withdraw_uuid, bal_in.amount
        )
        if acc is None:
            raise HTTPException(status_code=422, detail="Unprocessable Entity.")
        return acc
