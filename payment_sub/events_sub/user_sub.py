...
def process_user(msg):
    ...
    if val.get("name") == "user":
        ...
        sub_ev = sub_user_event.create(
            db,
            obj_in=SubUserEventCreate(event_id=event_id, user_id=user_id, state=state),
        )
        answer_msg = {"name": "payment-user", "user_id": user_id}
        ...
        if state == "new_user":
            ...
            logger.warn('"update_user" Not implemented')
        else:
