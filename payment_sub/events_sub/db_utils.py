...
class BalanceUtils:
            ...
            bal = (
                ...
                .filter(
                    and_(
                        ...
                        Balance.reserve is True,
                        Balance.success is True,
                    )
                )
                .all()
            )
    ...
    def cancel_reserved(
        ...
        with SessionLocal() as session:
            ...
            bal = (
                session.query(Balance)
                .filter(and_(Balance.order_uuid == order_uuid, Balance.reserve is True))
                .order_by(Balance.created_at.desc())
                .first()
            )
            if bal is None:
            ...
            if new_balance < Decimal("0.00") or new_balance < bal.balance:
                ...
                logger.info(
                    f"New balance try to be negative. {order_uuid=} "
