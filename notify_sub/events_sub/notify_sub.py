...
def process_notify(msg):
    ...
    if val.get("name") == "order":
        ...
        answer_msg = {"name": "notify", "order_uuid": order_uuid, "user_id": user_id}
        if val.get("state") == "canceling":
            ...
            if "balance" in reason:
                MSG_ORDER_CANCELED = (
                    f"Dear {customer.capitalize()} customer, your order "
            ...
            notify.create(
                db,
                obj_in=Notify(
                    order_uuid=order_uuid, client_id=user_id, msg=MSG_ORDER_CANCELED
                ),
            )
            ...
            customer = u.username if u else "?"
            MSG_ORDER_PAID = (
                f"Dear {customer.capitalize()} customer, you have paid for "
                f"your order. Your goods will be delivered by our courier."
            )
            notify.create(
                db,
                obj_in=Notify(
                    order_uuid=order_uuid, client_id=user_id, msg=MSG_ORDER_PAID
                ),
            )
