...
class LogisticUtils():
    ...
    def reserve_courier(
        ...
        with SessionLocal() as session:
            cu = session.query(Courier) \
                    .filter(Courier.order_uuid == order_uuid).all()
            if len(cu):
            ...
            if c is None:
                ...
                logger.info(f'There is no available couriers')
                return False
    ...
    def cancel_reserved(
        ...
        with SessionLocal() as session:
            c = session.query(Courier) \
                            .filter(Courier.order_uuid == order_uuid).first()
            if c is None:
